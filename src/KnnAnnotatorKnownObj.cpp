...
class KnnAnnotatorKnownObj : public DrawingAnnotator
{
private:
...
public:
  ...
  KnnAnnotatorKnownObj(): DrawingAnnotator(__func__)
  {}
  ...
  TyErrorId initialize(AnnotatorContext &ctx)
  {
    ...
    rsObject = new RSKNN(default_k);
    rsObject->loadModelFile(trainKNN_matrix);
    rsObject->setLabels(classNrMapping, model_labels);
  ...
  TyErrorId processWithLock(CAS &tcas, ResultSpecification const &res_spec)
  {
    ...
    if(feature_use == "VFH" || feature_use == "CVFH") {
      ...
      rsObject->processPCLFeatureKNN(mode, feature_use, clusters, color, model_labels, tcas);
    }
    else if(feature_use == "BVLC_REF" || feature_use == "VGG16") {
      outInfo("Calculation starts with : " << mode << "::" << feature_use);
      rsObject->processCaffeFeatureKNN(mode, feature_use, clusters, color, model_labels, tcas);
    }
    else {
